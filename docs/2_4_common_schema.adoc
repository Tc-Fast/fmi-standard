...
NOTE: For clocked variables, <<dependencies>> must only list knowns with a genuine simultaneous algebraic relationship at the same super-dense time instant; activating <<Clock,Clocks>> are declared via <<clocks>> and must not be listed here.
An importer must only consider a declared dependency between two clocked variables as an algebraic loop candidate if their Clocks can be simultaneously active.
