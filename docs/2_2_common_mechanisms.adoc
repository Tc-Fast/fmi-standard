...
The functions <<fmi3GetNumberOfVariableDependencies>> and <<fmi3GetVariableDependencies>> must only be called if <<providesPerElementDependencies>> is `true`, and only for a dependent variable that is listed as an unknown in the <<ModelStructure>>:
...
For any other variable, the FMU must return <<fmi3Error>>.

The variables returned as independents are the knowns latexmath:[{\mathbf{v}_{\mathit{known}}}] of the respective <<ModelStructure>> element, as defined for the <<dependencies>> attribute.
The returned dependency information must be consistent with the <<ModelStructure>>: a variable must only be returned as an independent if it is listed in the <<dependencies>> attribute of the respective element, or if that attribute is not present.
...
The <<dependencies>> declared there are therefore a conservative over-approximation, which these functions only refine by resolving to individual array elements, taking the current size of arrays into account and reflecting the current operating point.
Consequently, fewer dependencies may be returned than declared in the <<ModelStructure>>, but never additional ones.
