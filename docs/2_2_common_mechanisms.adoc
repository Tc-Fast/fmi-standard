...
Declaring a variable as <<clocked-variable,clocked variable>> using the <<clocks>> attribute declares an activation dependency: such a clocked variable can only be <<get-and-set-variable-values,accessed>> when one of the referenced Clocks is active.
This also holds for <<clocked-variable,clocked variables>> of type <<Clock>>.
The <<clocks>> attribute does not declare an algebraic dependency; Clocks listed in <<clocks>> must not be listed in <<dependencies>>.

[NOTE]
====
<<dependencies>> for clocked variables must only list knowns with a genuine simultaneous algebraic relationship at the same super-dense time instant.
An importer must only consider a declared <<dependencies,dependency>> as a candidate for algebraic loop detection if the Clocks of the unknown and the known can be simultaneously active.
====
